...
async function decryptJSON(
  pack: { enc: string; v: number; iv: string; salt: string; data: string },
  password: string
) {
  ...
  for (const url of candidates) {
    try {
      ...
      if (raw && raw.enc && raw.data) {
        const pwd =
          localStorage.getItem("settings_boot_backup_password") ||
          localStorage.getItem("settings_backup_password") ||
          "";
        if (!pwd) continue;
        try {
