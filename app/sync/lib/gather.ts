...
/** Helpers to scan and read LocalStorage keys into sections. */
...
export function groupKeys(keys: string[]): Section[] {
  ...
  return Object.entries(by).map(([id, ks]) => ({
...
export function applyBundle(bundle: any, mode: 'merge'|'replace' = 'merge'){
  ...
  if (mode==='replace'){
    // Remove any keys that exist in bundle target sections before writing
    for (const k of Object.keys(bundle.items)){
      localStorage.removeItem(k);
    }
  }
  for (const [k,v] of Object.entries(bundle.items)){
